...
SDI12::SDI12(int8_t dataPin, int8_t dir) {
  _dataPin        = dataPin;
  _DIR            = dir;
  _bufferOverflow = false;
  // SDI-12 protocol says sensors must respond within 15 milliseconds
...
void SDI12::begin(int8_t dataPin, int8_t dir) {
  _dataPin = dataPin;
  _DIR = dir;
  begin();
}
...
void SDI12::setState(SDI12_STATES state) {
  switch (state) {
    case SDI12_HOLDING: {
      ...
      pinMode(_DIR, OUTPUT);        // Pin mode = output
      digitalWrite(_dataPin, LOW);  // Pin state = low - marking
      digitalWrite(_DIR, LOW);      // Pin state = low - marking
    ...
    case SDI12_TRANSMITTING: {
      ...
      pinMode(_dataPin, OUTPUT);  // Pin mode = output
      pinMode(_DIR, OUTPUT);        // Pin mode = output
      digitalWrite(_DIR, HIGH);      // Pin state = low - marking
